$(function(){
    ...
    const loadMotion = () => {
        ...
        .addLabel('kvisual')
        .to(_slogan.find('.panel')[1], {duration:.3, className:'panel on', visibility:'inherit', opacity:1}, 'kvisual+=.2')
        ...
        gsap.set('.summary__arr', {y:'-1vw'});
        gsap.to('.summary__arr', .5, {y:'1vw', ease:Power1.easeOut, scrollTrigger:{
            trigger:$('.summary__arr'),
            start:'top 70%',
        }});

        /* infinite ani */
        gsap.to('.giveaway1', .7, {scale:1.05, ease:Power1.easeOut, yoyo:true, repeat:-1});
        gsap.to('.giveaway2', .7, {scale:1.15, ease:Power1.easeOut, yoyo:true, repeat:-1});
    }
                ...
                if(target == 'quiz_popup'){
                    ...
                    $('.submit_btn').on('click', function(){
                        ...
                        if($('input[name="quiz_agr"]').is(':checked') == false) {
                            _popup(`개인정보 수집 및 이용에 대한 동의<br>(동의합니다. 필수)`);
                            return;
                        }
                    ...
                    $('.url_submit').on('click', function(){
                        ...
                        _popup(`참여해 주셔서 감사합니다.<br>앞으로도 ROG x SON 캠페인에<br>많은 관심 부탁드립니다.`,'big');
                        selfClose(3000);
                    });
                }
            }
    ...
    $('.hash_btn').on('click', function(){
        ...
        _popup(`필수 해시태그가 복사되었습니다.<br>개인 SNS에 인증해주세요!`);
        selfClose(2000);
    });
    ...
    const scrollNav = () => {
        ScrollTrigger.create({
            //markers: {startColor:'#C500FF', endColor:'#FF4F00'},
            trigger: $('.rog__vis'),
            start: 'top 80%',
            onEnter: () => {
    ...
    const footHeight = () => {
        const $footer = $('.footer'),
              ...
              $wrapper = $('.wrap'),
              $w_height = $wrapper.outerHeight();

        let tl = new TimelineMax({
            scrollTrigger :{
                // markers:true,
                trigger:$('.wrap'),
                start: $w_height-$height +' 80%',
                end:'+='+$height +' 100%',
                scrub:.5,
        ...
        tl.to($footer, {duration:2.5, height: $height});
    }
    ...
    const init = () => {
        ...
        var swiper = new Swiper(".rog__story__vis", {
            effect: "coverflow",
            grabCursor: true,
            centeredSlides: true,
            slidesPerView: "auto",
    ...
    $(window).on('load', function(){
        ...
        gsap.utils.toArray('.shuffle').forEach(function(elem) {
            ScrollTrigger.create({
                //markers:true,
                start:'top 70%',
                trigger: elem,
                onEnter: function() {
                    new Ticker( $(elem) ).loop();
                }
            });
        });
    }); 
